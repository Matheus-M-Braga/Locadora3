   ...
   <div class="container_master">
      <v-container>
         ...
         <v-row wrap class="table">
            <v-data-table dark :headers="headers" :items="books" :items-per-page="5" class="elevation-1" item-key="id"
            ...
            <v-card dark>
               ...
               <v-card-text>
                  <v-col>
                     ...
                     <v-col cols="12">
                        <v-text-field v-model="autor" :counter="30" label="Autor" required :error-messages="AutorError"
                           @input="$v.autor.$touch()" @blur="$v.autor.$touch()"></v-text-field>
                     </v-col>
                     <v-col cols="12">
                        <v-select v-model="editora" :items="listEdits" item-text="nome" label="Editora" required
                           :error-messages="EditoraError" @input="$v.editora.$touch()"
                           @blur="$v.editora.$touch()"></v-select>
                     </v-col>
                     <v-col cols="12">
                        <v-text-field v-model="lancamento" :counter="30" label="Lançamento" required
                           :error-messages="LancaError" @input="$v.lancamento.$touch()" @blur="$v.lancamento.$touch()"
                           type="number"></v-text-field>
                     </v-col>
                     <v-col cols="12">
                        <v-text-field v-model="quantidade" :counter="30" label="Quantidade" required
                           :error-messages="QuantiError" @input="$v.quantidade.$touch()" @blur="$v.quantidade.$touch()"
                           type="number"></v-text-field>
                     </v-col>
                  </v-col>
               </v-card-text>
...
export default {
   ...
   computed: {
      ...
      LancaError() {
         ...
         !this.$v.lancamento.required && errors.push('Informe o ano de Lançamento.')
         return errors
      },
      QuantiError() {
         const errors = []
   ...
   methods: {
      ...
      openModalEdit(livro) {
         ...
         const selectedEdit = this.listEdits.find((editora) => editora.nome === livro.editora.nome)
      ...
      confirm() {
         ...
         if (!this.$v.$error) {
            ...
            if (this.ModalTitle === "Adicionar Livro") {
               const selectedEdit = this.listEdits.find((editora) => editora.nome === this.editora)
               const novoLivro = {
                  nome: this.nome,
                  autor: this.autor,
      ...
      removerLivroDaLista(livroId) {
         this.books = this.books.filter((livro) => livro.id
            !== livroId);
      }
   },
}
