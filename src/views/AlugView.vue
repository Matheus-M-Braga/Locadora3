         ...
         <v-dialog v-model="dialog" persistent max-width="500px">
            <v-card dark>
               ...
               <v-card-text>
                  <v-col>
                     <v-col cols="12">
                        <v-select v-model="livro_id" :items="listBooks" item-text="nome" label="Livro" required
                           :error-messages="BookError" @input="$v.livro_id.$touch()"
                           @blur="$v.livro_id.$touch()"></v-select>
                     </v-col>
                     <v-col cols="12">
                        <v-select v-model="usuario_id" :items="listUsers" item-text="nome" label="Usuário" required
                           :error-messages="UserError" @input="$v.usuario_id.$touch()"
                           @blur="$v.usuario_id.$touch()"></v-select>
                     </v-col>
                     <v-col cols="12">
                        <v-text-field disabled v-model="data_aluguel" label="Data do Aluguel (Hoje)" required
                           :error-messages="AlugError" @input="$v.data_aluguel.$touch()" @blur="$v.data_aluguel.$touch()"
                           type="date"></v-text-field>
                     </v-col>
                     <v-col cols="12">
                        <v-text-field v-model="data_previsao" label="Previsão de Devolução" required
                           :error-messages="DevolError" @input="$v.data_previsao.$touch()"
                           @blur="$v.data_previsao.$touch()" type="date"></v-text-field>
                     </v-col>
                  </v-col>
               </v-card-text>
               <v-card-actions>
...
export default {
   ...
   computed: {
      ...
      BookError() {
         ...
         !this.$v.livro_id.maxLength && errors.push('O limite é de 30 caracteres.')
      ...
      UserError() {
         ...
         !this.$v.usuario_id.maxLength && errors.push('O limite é de 30 caracteres.')
      ...
      AlugError() {
         ...
         !this.$v.data_aluguel.maxLength && errors.push('O limite é de 30 caracteres.')
         !this.$v.data_aluguel.required && errors.push('Informe a data do aluguel.')
         return errors
      },
      DevolError() {
         const errors = []
         if (!this.$v.data_previsao.$dirty) return errors
         !this.$v.data_previsao.maxLength && errors.push('O limite é de 30 caracteres.')
